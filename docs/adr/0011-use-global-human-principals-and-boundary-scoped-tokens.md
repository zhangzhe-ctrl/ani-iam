...
A Tenant-owned Workload Principal belongs to exactly one Tenant and cannot acquire Memberships in several Tenants, Platform Membership, or Workload Grants. Cross-Tenant integrations create separate Workload Principals and credentials. Removing its only Membership disables the Workload and irreversibly revokes all of its API Keys. Removing a Human Membership revokes only Sessions and Tokens scoped to that Tenant; other Tenant and Platform access remains intact.
