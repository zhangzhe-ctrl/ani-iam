...
The user accepted the OIDC ownership amendment on 2026-09-09 in the WR-16 documentation review: IAM owns server-side OIDC flow/state/verifier, Provider authorization-code exchange, Identity validation/linking, and Session/Token creation. Gateway retains the exact public callback, browser Cookie/CSRF handling and transport adaptation; it forwards callback inputs through the frozen internal contract and does not run a second authentication state machine. This supersedes only the former Gateway-owned OIDC/code-exchange/Session-creation rule. The downstream Workload verification mechanisms remain separately tracked under D01/D02; the browser rules below remain accepted.
...
Gateway removes every client-supplied `x-ani-*` identity header and keeps its IAM-verified ingress decision context local. Under the proposed follow-on model, a Gateway business hop would let Core and Services construct local typed Workload Invocation only after verifying the direct Gateway Workload and separately supplied delegation evidence; exact peer/WAT verification remains pending its checkpoint. A forwarded Principal header or Gateway context is never trusted in either model.
...
The Refresh cookie expires no later than its Session's absolute deadline, while server Session state remains authoritative. Non-browser clients receive no JSON Refresh Token in the basic version: external CLI and SDK automation use a Tenant-owned Workload Principal's API Key; the proposed refoundation would use verified Workload Identity plus target-bound Workload Access Tokens for Platform-owned Workloads once accepted; a future Human CLI login would require a separate Device Authorization Flow decision.

The accepted owner transfer is recorded as D08 in the [current decisions register](../../.scratch/ani-iam-workload-refoundation/decisions.md). Its exact internal contract is frozen in the M1 contract item and tested with real IAM, Gateway and OIDC Provider processes, without requiring frontend UI. Exact first-administrator initiation, durable state and trusted callback mechanics remain D03 work and cannot waive one-time consumption, authentication, audit or Secret handling. The old detailed ceremony/PG-flow recipe is historical candidate input rather than a required complete M1 control plane.

After IAM completes the code exchange and Session creation, the fixed Gateway callback sets the Refresh cookie and responds with a `303` to a fixed Console or BOSS page. Tokens and authorization codes never enter the redirect URL. On refresh failure, `401` clears frontend Access Token state and returns to login, `503` shows a transient service failure without an infinite retry loop, and reuse presents a generic expired-session result without disclosing detection details. M1 verifies the corresponding HTTP/Cookie/error contract without UI; M2 verifies the frontend behavior.
