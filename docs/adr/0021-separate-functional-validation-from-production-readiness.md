...
Argon2id latency and memory gates, concurrent `CheckPermission` load gates, repository-wide `go test -race`, broad Fuzz campaigns, and production-scale concurrency suites are deferred until production-readiness hardening. They do not block the first functional implementation. Targeted concurrency tests and `go test -race` for security-critical state machines named by an accepted work item—including ledger replay, Credential rotation, authority-anchor versioning, Invitation/last-admin, recovery exactly-once, outbox and consumer transitions—remain functional-correctness gates for that work item and cannot be deferred by this ADR. Until the broader hardening completes, its status remains `not_verified`; it is required before any production-readiness or production-release claim. Passing the targeted gates must not be described as production load, full race-safety, or abuse-resistance validation.
