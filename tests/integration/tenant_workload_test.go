...
func TestTenantWorkloadCreateUsesRestrictedPostgresAndOneAtomicTenantBoundary(t *testing.T) {
	...
	usecase := biz.NewTenantWorkloadUsecase(
	...
	result, err := usecase.CreateTenantWorkload(ctx, mustTenantScope(t, tenantID), biz.CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
	...
	if err := environment.runtimePool.QueryRow(ctx, `
		...
		FROM principals p
		JOIN workload_principals sp ON sp.principal_id=p.id
	...
	`, result.Principal.ID, result.AuditEventID).Scan(
		...
		t.Fatalf("query atomic tenant workload state: %v", err)
	}
	if principalType != "workload" || principalStatus != "active" || profileTenantID != tenantID || profileName != "external sdk" || normalizedName != "external sdk" {
...
func TestAPIKeyCreatePersistsDigestAndAuditWithoutRawSecret(t *testing.T) {
	...
	principal, err := usecase.CreateTenantWorkload(ctx, mustTenantScope(t, tenantID), biz.CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
...
func TestRemovingTenantWorkloadMembershipDisablesPrincipalAndRevokesEveryKey(t *testing.T) {
	environment := newPostgresEnvironment(t)
	ctx := context.Background()
	...
	tenantWorkloads := biz.NewTenantWorkloadUsecase(
		data.NewPostgresTenantWorkloadUnitOfWork(dataSet), data.NewUUIDv7Generator(), data.NewSystemClock(), allowingAPIKeyCreationLimiter{},
	)
	actor := biz.TenantAuthorizationActor{
		PrincipalID: actorID, AuthenticationMethod: biz.AuditAuthenticationMethodPassword,
		RequestID: "remove-sp", CorrelationID: "remove-sp", DecisionID: "remove-sp",
	}
	created, err := tenantWorkloads.CreateTenantWorkload(ctx, mustTenantScope(t, tenantID), biz.CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
	...
	for index := 0; index < 2; index++ {
		if _, err := tenantWorkloads.CreateAPIKey(ctx, mustTenantScope(t, tenantID), biz.CreateAPIKeyCommand{
	...
	if _, err := tenantAuthorization.UpdateMembership(ctx, mustTenantScope(t, tenantID), biz.UpdateTenantMembershipCommand{IdempotencyKey: uuid.NewString(),
	...
	if err := environment.runtimePool.QueryRow(ctx, `
		SELECT principal.status,membership.status,
		       ...
		       (SELECT count(*) FROM iam_audit_events audit
		         WHERE audit.tenant_id=$1 AND audit.action='iam.tenant-workload.disabled'
		           AND audit.target_id=$2)
		FROM principals principal
		JOIN workload_principals profile ON profile.principal_id=principal.id
		JOIN tenant_memberships membership ON membership.tenant_id=profile.tenant_id AND membership.principal_id=profile.principal_id AND membership.status='removed'
	...
	reenabled, err := tenantWorkloads.UpdateTenantWorkload(ctx, mustTenantScope(t, tenantID), biz.UpdateTenantWorkloadCommand{
		PrincipalID: created.Principal.ID, Status: biz.PrincipalStatusActive,
		ExpectedVersion: 2, IdempotencyKey: "reenable-sp", Actor: actor,
	})
	if err != nil {
		t.Fatalf("UpdateTenantWorkload(re-enable) error = %v", err)
	}
...
func TestAPIKeyAuthorizesTenantWorkloadThroughRestrictedPostgres(t *testing.T) {
	environment := newPostgresEnvironment(t)
	ctx := context.Background()
	...
	key, err := tenantWorkloads.CreateAPIKey(ctx, mustTenantScope(t, tenantID), biz.CreateAPIKeyCommand{
	...
	if !decision.Allowed || decision.DecisionID != decisionID || decision.Principal.ID != created.Principal.ID || decision.Principal.Type != biz.PrincipalTypeWorkload || decision.Principal.SessionID != uuid.Nil || decision.Principal.GrantID != uuid.Nil {
...
func TestAPIKeyRealPostgresValidationAndAuthorizationFailureMatrix(t *testing.T) {
	...
	principal, err := principalUsecase.CreateTenantWorkload(ctx, mustTenantScope(t, tenantA), biz.CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
	...
	principalB, err := principalUsecase.CreateTenantWorkload(ctx, mustTenantScope(t, tenantB), biz.CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
...
func TestTenantWorkloadManagementRealPostgresTenantPaginationConcurrencyAndRollback(t *testing.T) {
	environment := newPostgresEnvironment(t)
	ctx := context.Background()
	redisClient := newVerticalSliceRedisClient(t, ctx)
	...
	seedFormalHumanCredential(t, environment, actorA, tenantA, roleA, "management-a@example.test", "management-fixture-password")
	seedFormalHumanCredential(t, environment, actorB, tenantB, roleB, "management-b@example.test", "management-fixture-password")
	seedFormalIngress(t, environment)
	runtime := startFormalIAM(t, environment, redisClient)
	admin := runtime.admin
	loginA := formalLogin(t, runtime, tenantA, "management-a@example.test", "management-fixture-password")
	loginB := formalLogin(t, runtime, tenantB, "management-b@example.test", "management-fixture-password")
	ctxA := context.WithValue(ctx, formalCredentialKey{}, loginA.GetAccessToken())
	ctxB := context.WithValue(ctx, formalCredentialKey{}, loginB.GetAccessToken())
	createPrincipal := func(callContext context.Context, tenantID, roleID uuid.UUID, name, key string) *iamv1.TenantWorkload {
		response, createErr := admin.CreateTenantWorkload(callContext, &iamv1.CreateTenantWorkloadRequest{
	...
	gotA1, err := admin.GetTenantWorkload(ctxA, &iamv1.GetTenantWorkloadRequest{PrincipalId: principalA1.GetPrincipalId()})
	if err != nil || gotA1.GetPrincipal().GetTenantId() != tenantA.String() || gotA1.GetPrincipal().GetName() != "alpha bot" {
		t.Fatalf("GetTenantWorkload(positive) = %#v, error = %v", gotA1, err)
	}
	updatedA1, err := admin.UpdateTenantWorkload(ctxA, &iamv1.UpdateTenantWorkloadRequest{
		PrincipalId: principalA1.GetPrincipalId(), Status: iamv1.PrincipalStatus_PRINCIPAL_STATUS_ACTIVE, ExpectedVersion: 1, IdempotencyKey: "update-alpha",
	})
	if err != nil || updatedA1.GetPrincipal().GetName() != "alpha bot" || updatedA1.GetPrincipal().GetVersion() != 2 {
		t.Fatalf("UpdateTenantWorkload(positive) = %#v, error = %v", updatedA1, err)
	}
	gotA1, err = admin.GetTenantWorkload(ctxA, &iamv1.GetTenantWorkloadRequest{PrincipalId: principalA1.GetPrincipalId()})
	if err != nil || gotA1.GetPrincipal().GetName() != "alpha bot" || gotA1.GetPrincipal().GetVersion() != 2 {
		t.Fatalf("GetTenantWorkload(updated) = %#v, error = %v", gotA1, err)
	}
	...
	firstPage, err := admin.ListTenantWorkloads(ctxA, &iamv1.ListTenantWorkloadsRequest{
	...
	secondPage, err := admin.ListTenantWorkloads(ctxA, &iamv1.ListTenantWorkloadsRequest{
	...
	for _, page := range []*iamv1.ListTenantWorkloadsResponse{firstPage, secondPage} {
	...
	for _, principal := range []*iamv1.TenantWorkload{principalA1, principalA2, principalA3} {
	...
	pastUsecase := biz.NewTenantWorkloadUsecase(
		data.NewPostgresTenantWorkloadUnitOfWork(dataSet), data.NewUUIDv7Generator(), fixedClock{now: seededAt}, allowingAPIKeyCreationLimiter{},
	)
	...
	staleUsecase := biz.NewTenantWorkloadUsecase(
		data.NewPostgresTenantWorkloadUnitOfWork(dataSet), data.NewUUIDv7Generator(), fixedClock{now: staleAt}, allowingAPIKeyCreationLimiter{},
	)
	stalePrincipal, err := staleUsecase.CreateTenantWorkload(ctx, mustTenantScope(t, tenantA), biz.CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
	...
	if err != nil || operationalSnapshot.StaleNonExpiringCount < 1 || operationalSnapshot.UnusualTenantWorkloadCount < 1 {
		t.Fatalf("operator API key snapshot = %#v, error = %v", operationalSnapshot, err)
	}

	_, err = admin.CreateTenantWorkload(ctxA, &iamv1.CreateTenantWorkloadRequest{
		TenantId: tenantB.String(), Name: "Cross Tenant", RoleIds: []string{roleB.String()}, IdempotencyKey: "cross-create",
	})
	assertIntegrationIAMError(t, err, codes.PermissionDenied, "PERMISSION_DENIED", map[string]string{"operation_id": "createTenantWorkload"})
	_, err = admin.ListTenantWorkloads(ctxA, &iamv1.ListTenantWorkloadsRequest{TenantId: tenantB.String()})
	assertIntegrationIAMError(t, err, codes.PermissionDenied, "PERMISSION_DENIED", map[string]string{"operation_id": "listTenantWorkloads"})
	_, err = admin.GetTenantWorkload(ctxA, &iamv1.GetTenantWorkloadRequest{PrincipalId: principalB.GetPrincipalId()})
	assertIntegrationIAMError(t, err, codes.NotFound, "NOT_FOUND", map[string]string{"resource_type": "tenant_workload", "resource_id": principalB.GetPrincipalId()})
	...
	_, err = admin.UpdateTenantWorkload(ctxA, &iamv1.UpdateTenantWorkloadRequest{
		PrincipalId: principalB.GetPrincipalId(), Status: iamv1.PrincipalStatus_PRINCIPAL_STATUS_ACTIVE,
	...
	for index := 0; index < 2; index++ {
		go func(index int) {
			...
			_, updateErr := usecase.UpdateTenantWorkload(ctx, scopeA, biz.UpdateTenantWorkloadCommand{
				PrincipalID: principalID, Status: biz.PrincipalStatusActive,
	...
	rollbackUsecase := biz.NewTenantWorkloadUsecase(
		data.NewPostgresTenantWorkloadUnitOfWork(dataSet), data.NewUUIDv7Generator(), data.NewSystemClock(), allowingAPIKeyCreationLimiter{},
	)
	_, err = rollbackUsecase.UpdateTenantWorkload(ctx, mustTenantScope(t, tenantA), biz.UpdateTenantWorkloadCommand{
		PrincipalID: rollbackPrincipalID, Status: biz.PrincipalStatusDisabled,
	...
	if err := environment.runtimePool.QueryRow(ctx, `
		...
		FROM workload_principals profile JOIN principals principal ON principal.id=profile.principal_id
		WHERE profile.tenant_id=$1 AND profile.principal_id=$2
