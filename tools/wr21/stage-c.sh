...
set +e
go test -json -tags=integration -count=1 -timeout=15m ./tests/integration -run '^TestWR21StageCRevocationFailureRecovery$' > "$WR21_RUN_DIR/private/stage-c.raw.jsonl" 2>&1
result=$?
set -e
python3 - <<'PY'
import json,os
