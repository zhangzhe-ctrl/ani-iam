#!/usr/bin/env python3
"""WR21 exact-input, single-dispatch remote runner; never retries execution."""
...
EVIDENCE=AUTHORITY/'.scratch/ani-iam-workload-refoundation/evidence/21-complete-tenant-workload-api-key'
def sha(b):return hashlib.sha256(b).hexdigest()
def main():
 p=argparse.ArgumentParser();p.add_argument('--command-file',type=Path,required=True);args=p.parse_args()
 assert '**Status:** claimed' in (AUTHORITY/'.scratch/ani-iam-workload-refoundation/issues/21-complete-tenant-workload-api-key.md').read_text()
 run='wr21-'+datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-'+uuid.uuid4().hex[:8]
...
trap 'exit 143' TERM
trap 'exit 130' INT
trap 'result=$?; printf "%s\n" "$result" > "$WR21_RUN_DIR/command.exit"; date -u +%FT%TZ > "$WR21_RUN_DIR/command.finished"' EXIT
export WR21_RUN_DIR="$PWD"
...
export PATH=$WR21_RUN_DIR/private/bin:/home/ubuntu/.local/share/ani-iam/bin:$GOROOT/bin:$PATH
