#!/usr/bin/env python3
"""Create or remove only the exact WR32-owned Session acceptance cluster."""
import base64
import datetime
import hashlib
import json
...
run = Path(os.environ['WR32_RUN_DIR'])
assert str(run).startswith('/home/ubuntu/workspace/ani-iam-runs/wr32-') and run.parent == Path('/home/ubuntu/workspace/ani-iam-runs')
source = Path(__file__).resolve().parent
budget = json.loads((source/'session-budget.json').read_text())
name = run.name.lower() + '-session'
network = 'ani-iam-' + run.name
assert os.environ['WR32_DOCKER_NETWORK'] == network
private = run/'private'
admin = private/'session-admin.kubeconfig'
access = private/'session-access'
record_path = run/'session-cluster.json'
kind = Path(os.environ['WR32_CACHE'])/'kind-v0.33.0'
...
def execute(args, data=None, timeout=120):
    ...
    if p.returncode:
        with (private/'session-cluster-errors.log').open('ab') as f:
            f.write(p.stderr)
        raise RuntimeError('WR32 cluster command failed; private error retained')
    return p.stdout
...
record = {'cluster':name,'network':network,'created_by':'WR32 Goal','budget_sha256':sha(source/'session-budget.json'),'fixture_template_sha256':sha(source/'session-fixtures.json'),'kind_sha256':sha(kind),'kubectl_sha256':sha(kubectl),'node_image':node_image,'node_image_id':image['Id'],'node_absent_before':True,'initial_available_kib':available,'started_at':datetime.datetime.now(datetime.timezone.utc).isoformat()}
save(record)
...
# The host holds the reviewed amd64 image, not every manifest-list platform.
# Import that exact platform through the node's existing containerd.
execute(['docker','exec','-i',record['node_id'],'ctr','--namespace=k8s.io','images','import','--platform','linux/amd64','--digests','--index-name',shell_image,'--snapshotter=overlayfs','-'],archive.read_bytes(),timeout=120)
aliases=execute(['docker','exec',record['node_id'],'ctr','--namespace=k8s.io','images','list','-q']).decode().splitlines()
assert shell_image in aliases, 'exact pinned shell image must be named in task containerd'
record['shell_image']=shell_image
record['shell_image_id']=shell['Id']
record['shell_archive_sha256']=sha(archive)
manifest = json.loads((source/'session-fixtures.json').read_text().replace('WR32_RUN_ID',run.name))
...
for suffix in ('000000000001','000000000002'):
    namespace='ani-tenant-01993000-0019-7000-8000-'+suffix
    k(['wait','pod','--all','-n',namespace,'--for=condition=Ready','--timeout=60s'])
ca=json.loads(k(['get','configmap','kube-root-ca.crt','-n','wr32-runtime','-o','json']))['data']['ca.crt']
...
tokens={}
for account in ['wr32-session','wr32-gateway-reader']:
    body={'apiVersion':'authentication.k8s.io/v1','kind':'TokenRequest','spec':{'expirationSeconds':3600}}
    tokens[account]=json.loads(k(['create','--raw','/api/v1/namespaces/wr32-runtime/serviceaccounts/'+account+'/token','-f','-'],json.dumps(body).encode()))['status']
access.mkdir(mode=0o700)
write_private(access/'ca.crt',ca.encode())
write_private(access/'gateway.token',tokens['wr32-gateway-reader']['token'].encode())
session={'apiVersion':'v1','kind':'Config','clusters':[{'name':name,'cluster':{'server':server,'certificate-authority-data':base64.b64encode(ca.encode()).decode()}}],'users':[{'name':'session','user':{'token':tokens['wr32-session']['token']}}],'contexts':[{'name':name,'context':{'cluster':name,'user':'session'}}],'current-context':name}
...
checks=[('wr32-session','/api/v1/namespaces/'+namespace+'/pods',200),('wr32-session','/api/v1/namespaces/default/pods',403),('wr32-session','/api/v1/namespaces/'+namespace+'/secrets',403),('wr32-gateway-reader','/apis/apps/v1/namespaces/'+namespace+'/deployments/wr19-exec-a',200),('wr32-gateway-reader','/api/v1/namespaces/'+namespace+'/pods',403)]
