#!/usr/bin/env python3
"""WR32 adaptation of tools/wr21/remote-run.py: fixed inputs, one remote command.
...
EVIDENCE=AUTHORITY/'.scratch/ani-iam-workload-refoundation/evidence/32-stabilize-workload-integration'
...
def main():
    ...
    assert '**Status:** claimed' in (AUTHORITY/'.scratch/ani-iam-workload-refoundation/issues/32-stabilize-workload-integration.md').read_text()
    run='wr32-'+datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-'+uuid.uuid4().hex[:8]
    local=EVIDENCE/'runs'/run;local.mkdir(parents=True)
    remote='/home/ubuntu/workspace/ani-iam-runs/'+run
    ssh=['ssh','-o','BatchMode=yes','-o','ConnectTimeout=12','-o','ServerAliveInterval=15','-o','ServerAliveCountMax=3','ubuntu']
    ...
    assert identity==['ubuntu','i-8yg2l7u8']
    record={'run_id':run,'remote':remote,'state':'preparing','command':str(args.command_file.resolve()),'repositories':{}}
    ...
    for name,baseline in claim['inputs'].items():
        ...
        subprocess.run(['scp','-q',str(archive),'ubuntu:'+remote+'/'],check=True)
        if args.git_metadata:
            ...
            subprocess.run(['scp','-q',str(pack),'ubuntu:'+remote+'/'+name+'-baseline.pack'],check=True)
            if name == 'ani':
                ...
                subprocess.run(['scp','-q',str(extra),'ubuntu:'+remote+'/ani-history.pack'],check=True)
...
umask 077
cd "$(dirname "$0")"
export WR32_RUN_DIR="$PWD"
echo $$ > command.pid
trap 'exit 143' TERM
trap 'exit 130' INT
trap 'rc=$?; echo "$rc" > "$WR32_RUN_DIR/command.exit"; date -u +%FT%TZ > "$WR32_RUN_DIR/command.finished"' EXIT
...
export GOCACHE="$WR32_CACHE/go-build" GOMODCACHE="$WR32_CACHE/go-mod" GOTMPDIR="$PWD/private/tmp" BUF_CACHE_DIR="$WR32_CACHE/buf"
export GOPATH="$WR32_CACHE/gopath"
...
'''
    ...
    runner=runner.replace('export GOCACHE=', 'export WR32_CACHE='+shlex.quote(claim['remote_root']+'/cache')+'\nexport GOCACHE=')
    (local/'runner.sh').write_text(runner)
    for name in ['run.json','command.sh','audit-source.py','runner.sh']:
        subprocess.run(['scp','-q',str(local/name),'ubuntu:'+remote+'/'],check=True)
