...
go test -count=1 ./internal/biz ./internal/data ./internal/service ./internal/server ./internal/conf ./cmd/server ./tests/contracts > "$WR22_RUN_DIR/private/iam-administration-check.log" 2>&1
...
(cd ../source-ani/repo/services/ani-gateway && go test -count=1 ./internal/authz ./internal/middleware ./internal/router && go build -o "$WR22_RUN_DIR/private/ani-gateway" .) > "$WR22_RUN_DIR/private/gateway-check.log" 2>&1
set +e
go test -json -tags=integration -count=1 -timeout=12m ./tests/integration -run "${WR22_FORMAL_TESTS:-^TestWR22FormalTenant(Roles|Membership|Audit)$}" > "$WR22_RUN_DIR/private/roles-formal.log" 2>&1
result=$?
set -e
printf 'roles-formal %s\n' "$result" >> "$WR22_RUN_DIR/roles-check-exits.txt"
python3 - <<'PY'
import os,json
