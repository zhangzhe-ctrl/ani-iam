#!/usr/bin/env python3
"""Collect an existing WR22 run. Never rerun commands or overwrite newer edits."""
import argparse,hashlib,json,subprocess,tarfile,io,stat
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]
E=ROOT.parent/'ani-iam/.scratch/ani-iam-workload-refoundation/evidence/22-complete-identity-administration'
def sha(data):return hashlib.sha256(data).hexdigest()
def main():
 p=argparse.ArgumentParser();p.add_argument('run');p.add_argument('--generated',action='store_true');a=p.parse_args()
 assert a.run.startswith('wr22-') and '/' not in a.run
 ...
 for name in ['command.pid','command.started','command.exit','command.finished','tools.txt','notification-tools.txt','notification-ripgrep.txt','generation.json','stage-a-results.json','stage-b-results.json','stage-c-results.json','reference-events.jsonl','resources.jsonl','network-stopped.log','check-exits.txt','envoy-version.txt','phase-exits.txt','gates-exits.txt','gates-network-stopped.log','example-processes.jsonl','operator-stop.json','roles-results.json','roles-check-exits.txt','audit-mutant.json','audit-mutant.patch','audit-mutant-source.json']:
 ...
 scopes={k:json.loads((E/(k+'-scope.json')).read_text()) for k in ('iam','ani','notification') if (local/(k+'-source.json')).exists()}
 ...
 for k,manifest in manifests.items():
  ...
  for name,row in manifest.items():
   relevant=(k=='iam' and (name.endswith(('.proto','.sql')) or 'buf.' in name or name=='sqlc.yaml' or name=='internal/data/cmd/genoperationregistry/main.go')) or (k=='ani' and name in ['repo/scripts/generate_gateway_authz.py','repo/scripts/gen_sdk_alpha.py','repo/scripts/generate_api_docs.py','repo/scripts/wr22_inference_contract.py','repo/api/openapi/v1.yaml','repo/api/openapi/services/v1.yaml','repo/api/openapi/wr19-workload-operation-registry.v1.json','repo/services/ani-gateway/tools/wr22_operation_registry.py','repo/services/ani-gateway/tools/dp2_operation_registry.py','repo/api/proto/inference/control/v1/inference_control.proto','repo/api/proto/buf.yaml','repo/api/proto/buf.gen.yaml'])
   if k=='notification' and (name.endswith('.proto') or name in ['buf.yaml','buf.gen.yaml','buf.lock','go.mod','go.sum']):relevant=True
