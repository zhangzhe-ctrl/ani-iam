...
go test -count=1 ./internal/biz ./internal/data ./internal/service ./internal/server ./cmd/server ./tests/contracts > "$WR22_RUN_DIR/private/iam-administration-check.log" 2>&1
...
set +e
go test -json -tags=integration -count=1 -timeout=12m ./tests/integration -run '^TestWR22FormalFirstAdministratorRegistration$' > "$WR22_RUN_DIR/private/roles-formal.log" 2>&1
result=$?
set -e
printf 'first-administrator-registration %s\n' "$result" >> "$WR22_RUN_DIR/roles-check-exits.txt"
python3 - <<'PY'
import os,json
