...
go test -tags=integration -run '^TestFormalWorkloadInvocation$' -count=1 -timeout=10m -json ./tests/integration > "$WR19_RUN_DIR/continuation-integration.jsonl"
cd "$WR19_RUN_DIR"
...
go test ./...
go vet ./...
go test -race ./internal/session ./internal/transport/websocket/connectedsession ./internal/transport/grpc
