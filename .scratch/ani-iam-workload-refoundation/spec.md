...
> 2026-09-16 当前目标修订：Tenant 生命周期及后续治理 owner 改为独立 Governance；采用单一目标合同，不维护旧 Core/Governance 双协议或 fallback。用户 Goal 已授权并领取 [WR33](issues/33-integrate-governance.md) 实施 I1～I4；WR24 保存 checkpoint 后释放领取并受其阻塞，产品执行仍暂停。有限 IAM 适配、与 GOV-01～03 并行、WR24/GOV-04 汇合和后续 M1/M2 顺序见 [并行实施计划](../../docs/plans/iam-governance-parallel-transition.md)。未逐项修订的历史 Core owner 措辞按该目标映射解释；已接受安全/身份流程和全部验收分母保留，历史证据不改写。新联合验证尚未完成。
...
将 ANI 的 auth-service 与用户、成员、权限管理抽离为独立 IAM，职责更清晰，调用方只依赖稳定契约；Tenant 生命周期、套餐和 Quota 治理由独立 Governance 拥有，保留资源服务自己的对象、执行与恢复。首期租户创建不以套餐/Quota 实现为前置，也不限定 Governance 的长期职责。
...
- [API 授权声明规范](../../docs/api-authorization-contract.md)：owner/IAM/Gateway 职责、声明与角色约束、生成和执行边界，以及整体切换时的逐接口核对要求；不替代当前能力分母或冻结契约。
...
| 领域 | 唯一负责的事实与事务 | 对外关系 |
| --- | --- | --- |
| Governance | Tenant ID/业务资料/Lifecycle 及自己的 outbox；套餐与 Quota 是后续职责，不是首期创建前置 | 发布自有生命周期事实、引用 IAM Bootstrap 请求；提供有限 REST Snapshot；不管理 IAM 凭据/成员/角色 |
| IAM | Human/Workload、Identity/Credential、Session、Access、Membership、Role/Binding、Invitation、权限评估与安全 Audit | 通过固定版本契约提供认证、管理和决定；不写 Governance/资源数据库 |
...
2026-09-13 用户确认的稳定性要求：同一种已支持安全机制的新服务，通过 owner 契约、受审核的目标注册、独立身份/Grant 和 owner adapter 接入；不得要求修改 IAM 手写业务代码、数据库结构或通用 SDK。注册只描述精确目标与约束，不授予权限；验证入口权限不等于任意 audience 的 receiver 权限。新增安全机制本身仍属于 IAM 业务变更，不能把任意脚本、通配权限或动态策略引擎作为“通用化”。现有同步机制的有界纠偏单列 [WR32](issues/32-stabilize-workload-integration.md)，先于 WR23 剩余接线；不继续为 Core Snapshot 等目标增加服务名特例。

本期保留 Console/BOSS 和既有 OIDC 上游范围；多 Provider、Browser Client 注册、通用 OIDC Provider、SCIM/SAML 不进入 WR32 或 WR23–28 的完成条件。当前范围、成本估计和后续收口要求集中在[收口报告](evidence/23-integrate-core-lifecycle-bootstrap/scope-and-closeout-review.md)。
...
实际消费者切换需固定最终组合并验证旧 writer 退出。按用户已明确的舍弃旧数据策略，WR28 采用新库初始化与一次性实际切换，不建设旧数据迁移、双写、旧身份兼容或恢复旧数据；正常启用后产生的新数据另行保护。切流、Credential 失效、数据重建、物理删除按各自精确事项和人工确认执行。必要的新系统初始化、权限隔离、审计和失败处理保留；未接受的完整 Lineage/PONR/长期 DR 不作为当前前置。具体边界见 [WR28](issues/28-complete-system-replacement.md)。
...
整理任务 WR-16 的完成仅说明文档交付与审查完成，不等于 M1 pass，也不替用户接受未决技术机制。WR-17/18 的合同冻结与隔离运行基础见 [历史交接](evidence/18-refound-human-workload-runtime/README.md)；此后各票状态只查 [当前事项图](ticket-plan.md)，本节不重复维护进度。
