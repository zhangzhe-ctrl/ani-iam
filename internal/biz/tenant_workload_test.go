...
func TestTenantWorkloadCreateCommitsProfileMembershipBindingsAndAuditAtomically(t *testing.T) {
	...
	tx := &recordingTenantWorkloadTransaction{
		roles: map[uuid.UUID]TenantRole{roleID: {ID: roleID, Code: "sdk-client", Version: 1}},
	}
	usecase := NewTenantWorkloadUsecase(
		&recordingTenantWorkloadUnitOfWork{tx: tx},
		&fixedIDs{values: []uuid.UUID{principalID, membershipID, bindingID, auditID}},
		fixedAuthClock{now: now}, allowingAPIKeyCreationLimiter{})

	result, err := usecase.CreateTenantWorkload(context.Background(), scope, CreateTenantWorkloadCommand{IdempotencyKey: uuid.NewString(),
	...
	if tx.mutation.Audit.ID != auditID || tx.mutation.Audit.Action != AuditActionTenantWorkloadCreated || tx.mutation.Audit.TargetID != principalID || tx.mutation.Audit.TargetVersion != 1 {
...
func TestAPIKeyCreateReturnsSecretOnceAndCommitsOnlyDigestWithAudit(t *testing.T) {
	...
	tx := &recordingTenantWorkloadTransaction{
		principals: map[uuid.UUID]TenantWorkload{principalID: {
...
func TestAPIKeyCreateRequiresExactlyOneExpiryMode(t *testing.T) {
	...
	usecase := NewTenantWorkloadUsecase(
		&recordingTenantWorkloadUnitOfWork{tx: &recordingTenantWorkloadTransaction{}},
...
func TestAPIKeyCreateRateLimitRunsBeforeSecretGeneration(t *testing.T) {
	...
	tx := &recordingTenantWorkloadTransaction{
		principals: map[uuid.UUID]TenantWorkload{principalID: {ID: principalID, Status: PrincipalStatusActive, Version: 1}},
	}
	limiter := &recordingAPIKeyCreationLimiter{err: rateLimit}
	usecase := NewTenantWorkloadUsecase(
		&recordingTenantWorkloadUnitOfWork{tx: tx}, &fixedIDs{},
...
func TestTenantWorkloadDisableRevokesEveryActiveKeyWithAudit(t *testing.T) {
	...
	tx := &recordingTenantWorkloadTransaction{principals: map[uuid.UUID]TenantWorkload{principalID: {
		ID: principalID, MembershipID: membershipID, Name: "Build Bot", NormalizedName: "build bot",
		Status: PrincipalStatusActive, Version: 1,
	}}}
	usecase := NewTenantWorkloadUsecase(
		&recordingTenantWorkloadUnitOfWork{tx: tx}, &fixedIDs{values: []uuid.UUID{auditID}},
		fixedAuthClock{now: now}, allowingAPIKeyCreationLimiter{})

	result, err := usecase.UpdateTenantWorkload(context.Background(), scope, UpdateTenantWorkloadCommand{
		PrincipalID: principalID, Status: PrincipalStatusDisabled,
		ExpectedVersion: 1, IdempotencyKey: "disable-sp-1", Actor: validTenantAuthorizationActor(),
	})
	if err != nil {
		t.Fatalf("UpdateTenantWorkload() error = %v", err)
	}
	if result.Principal.Status != PrincipalStatusDisabled || result.Principal.Name != "Build Bot" || result.RevokedAPIKeys != 4 || tx.tenantWorkloadUpdates != 1 {
		t.Fatalf("result/updates = %#v/%d", result, tx.tenantWorkloadUpdates)
	}
	if tx.tenantWorkloadAudit.Action != AuditActionTenantWorkloadDisabled || tx.tenantWorkloadAudit.TargetVersion != 2 {
		t.Fatalf("audit = %#v", tx.tenantWorkloadAudit)
	}
}
...
func TestAPIKeyRevokeIsLifecycleIdempotent(t *testing.T) {
	...
	tx := &recordingTenantWorkloadTransaction{apiKeys: map[uuid.UUID]APIKey{keyID: {
...
type recordingTenantWorkloadUnitOfWork struct {
	tx *recordingTenantWorkloadTransaction
}

func (u *recordingTenantWorkloadUnitOfWork) WithinTenantWorkload(ctx context.Context, _ TenantScope, fn func(context.Context, TenantWorkloadTransaction) error) error {
...
type recordingTenantWorkloadTransaction struct {
	...
	commits               int
	mutation              TenantWorkloadCreateMutation
	apiKeyMutation        APIKeyCreateMutation
	apiKeys               map[uuid.UUID]APIKey
	tenantWorkloadUpdates int
	tenantWorkloadAudit   SecurityAuditEvent
	apiKeyRevokes         int
	credential            APIKeyCredential
	credentialIssueCalls  int
}

func (t *recordingTenantWorkloadTransaction) GetRole(_ context.Context, _ TenantScope, roleID uuid.UUID) (TenantRole, error) {
	return t.roles[roleID], nil
}

func (t *recordingTenantWorkloadTransaction) CreateTenantWorkload(_ context.Context, _ TenantScope, mutation TenantWorkloadCreateMutation) error {
	t.mutation = mutation
	return nil
}

func (t *recordingTenantWorkloadTransaction) GetTenantWorkload(_ context.Context, _ TenantScope, principalID uuid.UUID) (TenantWorkload, error) {
	principal, ok := t.principals[principalID]
	if !ok {
		return TenantWorkload{}, ErrTenantWorkloadNotFound
	}
	return principal, nil
}

func (t *recordingTenantWorkloadTransaction) IssueAPIKeyCredential(_ context.Context, keyID uuid.UUID) (APIKeyCredential, error) {
...
func (t *recordingTenantWorkloadTransaction) CreateAPIKey(_ context.Context, _ TenantScope, mutation APIKeyCreateMutation) error {
	t.apiKeyMutation = mutation
	return nil
}

func (t *recordingTenantWorkloadTransaction) UpdateTenantWorkload(_ context.Context, _ TenantScope, principal TenantWorkload, _ int64) (TenantWorkload, int64, error) {
	t.tenantWorkloadUpdates++
	principal.Version++
	t.principals[principal.ID] = principal
	return principal, 4, nil
}

func (t *recordingTenantWorkloadTransaction) GetAPIKey(_ context.Context, _ TenantScope, keyID uuid.UUID) (APIKey, error) {
...
func (t *recordingTenantWorkloadTransaction) RevokeAPIKey(_ context.Context, _ TenantScope, key APIKey, _ int64) (APIKey, error) {
	t.apiKeyRevokes++
...
func (t *recordingTenantWorkloadTransaction) AppendAudit(_ context.Context, _ TenantScope, audit SecurityAuditEvent) error {
	t.tenantWorkloadAudit = audit
	return nil
}

func (t *recordingTenantWorkloadTransaction) CreateCurrentWorkloadMembership(_ context.Context, _ TenantScope, membership TenantMembership) error {
	principal := t.principals[membership.PrincipalID]
