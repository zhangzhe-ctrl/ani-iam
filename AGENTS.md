...
在本仓库开始工作前，必须阅读并遵守 [CLAUDE.md](CLAUDE.md)。

本文件只保留稳定入口，不记录当前票号或进度。当前范围与顺序唯一入口为 [.scratch/ani-iam-workload-refoundation/spec.md](.scratch/ani-iam-workload-refoundation/spec.md) 和 [ticket-plan.md](.scratch/ani-iam-workload-refoundation/ticket-plan.md)；领域词汇在 [CONTEXT.md](CONTEXT.md)，已接受理由在 [docs/adr/](docs/adr/)。历史 CP0/Direct P2/WR 旧稿仅供追溯。

只读分析可直接进行。改变仓库或外部状态前，必须有明确范围、基线、依赖、验收、测试、恢复和停止条件的唯一 `claimed` 事项。删除旧运行资产、切流、Credential 失效、数据重建、生产操作或其他难以恢复动作，须在执行前取得精确人工确认。
